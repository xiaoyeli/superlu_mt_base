...
void
@(pre)gstrs(trans_t trans, SuperMatrix *L, SuperMatrix *U, 
 ...
 *         p@(pre)gstrf(). Use compressed row subscripts storage for supernodes,
 *         i.e., L has types: Stype = SCP, Dtype = _D, Mtype = TRLU.
 *
 * U       (input) SuperMatrix*
 *         The factor U from the factorization Pr*A*Pc=L*U as computed by
 *         p@(pre)gstrf(). Use column-wise storage scheme, i.e., U has types:
...
@precision !
		...
		for (j = 0, bptr = fsupc; j < nrhs; j++, bptr += ldb) {
		    @(pre)usolve (nsupr, nsupc, &Lval[luptr], &Bmat[bptr]);
		}
#endif		
	    }
...
@precision DCOMPLEX SCOMPLEX
        if (trans == TRANS) {
            ...
            for (k = 0; k < nrhs; ++k) {
                ...
                sp_@(pre)trsv("U", "C", "N", L, U, &Bmat[k*ldb], info);

                /* Multiply by conj(inv(L')). */
                sp_@(pre)trsv("L", "C", "U", L, U, &Bmat[k*ldb], info);
            }
         }
@precision !
